...
SECTION Safety framing: Why -19 is the point of no return, what you have already accepted (-15, -18), and what still stands between here and the flip (-12, -24, and read/bridge health). Corrects the abort story: there are TWO distinct abort paths and they are NOT the same speed. The FAST in-incident lever is the runtime kill-switch (the admin page writes a persistent config doc that every dispatch re-reads, so it takes effect immediately across all pods with NO restart) — but it only exists AFTER the -19 flip and the admin page is iot-role-gated. The SLOW path is re-engaging WRITES_DISABLED=true, which needs a manifest edit plus a rolling pod restart, during which some pods still dispatch (partial-state window).
SECTION Conditions carried forward: This plan was ACCEPTED WITH CONDITIONS by the operator after a governed-panel review. Every finding raised by the critical-thinker and customer-experience panels is recorded as a named, owned condition and attached to the downstream gate that must resolve it before proceeding. The register lists each condition honestly as open/tracked — NOT solved — with who raised it, which gate carries it, its owner, and the point by which it must be resolved.
DECISION 1: Approve this shaping and stage-entry plan as the basis for the group release. OUTCOME: ACCEPTED WITH CONDITIONS — approval holds on condition that every register row below is carried into and resolved at its named gate; none of these conditions is treated as already solved.
...
> **Operator decision: ACCEPTED WITH CONDITIONS (2026-09-11).** After a governed-panel review (critical-thinker + customer-experience), James approved this shaping plan **on condition that every panel finding is recorded as a named, owned condition and carried into the downstream gate that must resolve it.** Those conditions are captured in **§10, "Conditions Carried Forward"** — each one is honestly logged as **open / tracked, not solved**, and gates the stage it is attached to. Approval of Decision 1 holds only while those conditions are carried forward and cleared at their named gates.
...
**Verified read-timing (matters for how fast the flip — and each abort path — takes effect):** `config.js` builds `export const config = {...}` **once at module import**, evaluating `writesDisabled: env('WRITES_DISABLED') === 'true'` a **single time at process start**. The env value is therefore **cached at pod startup, not read per request**. Consequence for the flip: the OOHDASH-19 flip requires **editing the manifest env AND performing a rolling pod restart** to take effect — it is **not instantaneous**. **Important correction (see §9): the env flag is NOT the fast abort lever.** There are **two distinct abort paths and they are not the same speed** — the **runtime kill-switch is fast** (it writes a persistent config doc that every dispatch re-reads, so it applies immediately across all pods with no restart) while the **env re-disable is slow** (manifest edit + rolling pod roll, with a partial-state window while the roll completes). This shapes what the Release-preflight and the shakedown abort trigger must expect (see §9).
...
**Two abort paths — and they are NOT the same speed.** Verified in `services/killswitch.js`: `writesBlocked()` checks `config.writesDisabled` (the cached env value) **first and synchronously**, and **then** reads the `OohAppConfig` doc from the persistent store (Cosmos) on **every** dispatch for the runtime kill-switch. That order gives you two genuinely different levers:

- **(a) Runtime kill-switch — the FAST, in-incident lever.** The admin page's global kill-switch **writes the persistent `OohAppConfig` doc** (`setGlobal` in `services/killswitch.js`). Because every dispatch **re-reads that doc**, engaging it takes effect **immediately across all pods with NO restart**. Two caveats stated plainly: it only exists **after** the -19 flip (while `WRITES_DISABLED=true` the deploy lock short-circuits first, so the kill-switch is moot until the flip), and the **admin page is iot-role-gated** — so the person who will pull it must hold the `iot` role.
- **(b) `WRITES_DISABLED=true` env revert — the SLOW, durable backstop.** Re-engaging the deploy lock needs a **manifest edit plus a rolling pod restart**. As established in §1 the env value is cached at pod startup, so during the roll **some pods still dispatch** — a **partial-state window** until every pod has cycled. There is **no instant off switch at the env level**.

**Do not conflate the two:** the fast lever is the **runtime kill-switch**, not the env flag. The env re-disable is the durable backstop that lands at pod-roll speed. Release-preflight must confirm **both** paths are understood before go — and, per the register (§10), that the shakedown supervisor actually holds the `iot` role needed to operate path (a).
...
- **A re-disable / abort trigger** — the named condition that halts the shakedown and the agreed re-disable path. Abort uses **the runtime kill-switch first** (immediate, all-pods, no restart — the fast lever of §9 path (a)), with the **env re-disable as the durable backstop** (pod-roll speed, partial-state window — path (b)). The abort SLA is therefore driven by the **measured pod-roll duration** for the backstop, which must be measured and stated in advance (see §10).

---

## 10. Conditions Carried Forward (accept-with-conditions register)

This plan was **accepted with conditions** by the operator after a governed-panel review. Every finding raised by the critical-thinker and customer-experience panels is recorded below as a **named, owned condition**, attached to the downstream gate that must resolve it. **Each row is honestly logged as open / tracked — none is treated as solved.** Approval of Decision 1 holds only while these are carried forward and cleared at their named gates.

**Status of every row below: OPEN / TRACKED** (not resolved).

<table>
<thead>
<tr><th>Condition</th><th>Raised by</th><th>Carried to gate</th><th>Owner</th><th>Must be resolved before</th></tr>
</thead>
<tbody>
<tr><td>Establish the <strong>true count of sign-in-eligible claimArea-1500 accounts</strong> (and who they are) — every one gains dispatch at the flip; the number is unproven today. Decision 4 cannot be taken until this exists.</td><td>critical-thinker</td><td>Discovery</td><td>James / Spencer</td><td>Design gate (before Decision 4 is put)</td></tr>
<tr><td><strong>Supervised shakedown fully specified</strong> — named supervisor, isolated / bench device for first actuation, explicit success criteria, re-disable / abort trigger, and the partial-state-during-roll window explicitly acknowledged.</td><td>critical-thinker</td><td>Release-preflight</td><td>James</td><td>OOHDASH-19 flip</td></tr>
<tr><td><strong>Bridge-read gap raised as an owned OOHDASH ticket</strong> (site-search 503 / F025 device contract) — blocks reaching any device at all.</td><td>critical-thinker</td><td>Prerequisite (before Discovery starts)</td><td>Spencer / IoT platform</td><td>Discovery begins</td></tr>
<tr><td><strong>OOHDASH-12 environment-config audit</strong> — dev / staging / CI + any compose or local <code>.env</code> templates + the k8s manifest — before inverting the fail-open default silently changes behaviour anywhere it is omitted.</td><td>critical-thinker</td><td>Build-PLAN</td><td>build doer</td><td>OOHDASH-12 merges</td></tr>
<tr><td><strong>iot-only rework scope characterised</strong> — if Decision 4 goes iot-only, the change to the exact-match role check may touch every guarded route; the scope must be sized, not assumed small.</td><td>critical-thinker</td><td>Discovery / Design (for -67)</td><td>design doer</td><td>Build of -67</td></tr>
<tr><td><strong>Pod-roll (rolling restart) duration measured and stated</strong> as the abort SLA input for the env re-disable backstop (§9 path b).</td><td>critical-thinker</td><td>Release-preflight</td><td>James / ops</td><td>Shakedown spec is finalised</td></tr>
<tr><td><strong>Shakedown supervisor confirmed to hold the <code>iot</code> role</strong> — the kill-switch admin page (the fast abort lever, §9 path a) is iot-gated, so the person who will pull it must have that role.</td><td>critical-thinker</td><td>Release-preflight</td><td>James</td><td>Shakedown</td></tr>
<tr><td><strong>Isolated bench / test Salus device confirmed available</strong> for the first actuation (not a live trading site).</td><td>critical-thinker</td><td>Prerequisite (alongside bridge-read)</td><td>IoT team</td><td>Release-preflight</td></tr>
<tr><td><strong>Handler live-control UX confirmed present or ticketed</strong> — pre-dispatch confirmation, post-dispatch feedback, and a device-non-responsive state — AND named explicitly as a Release-preflight gate condition.</td><td>customer-experience</td><td>Design</td><td>design doer</td><td>Release-preflight</td></tr>
<tr><td><strong>"Control is now live" in-app signal treated as a HARD Design-gate requirement</strong> — not an optional call. A handler must not be unaware they now hold device-control access at the flip.</td><td>customer-experience</td><td>Design</td><td>design doer</td><td>Design gate sign-off</td></tr>
<tr><td><strong>Handler-communication plan for the chosen -67 authz policy</strong> — how handlers are told they now hold dispatch, for whichever policy Decision 4 lands on.</td><td>customer-experience</td><td>Design</td><td>James</td><td>Design gate sign-off</td></tr>
<tr><td><strong>OOHDASH-11 man-marking scenarios explicitly cover handler-UX comprehension</strong> — not only correct actuation; the scripts must prove the handler understood the UX, not just that the command fired.</td><td>customer-experience</td><td>Writer</td><td>IoT lead</td><td>OOHDASH-11 accept (before shakedown)</td></tr>
</tbody>
</table>

**Cost / benefit of carrying these forward rather than blocking now.** *Benefit:* approval can proceed today without pretending open risks are closed — each risk is owned, attached to the gate with the context to resolve it, and visible to the operator on one page. *Cost:* twelve conditions must be actively tracked to closure across Discovery, Design, Build-PLAN and Release-preflight; if any is silently dropped, the accept-with-conditions basis of Decision 1 no longer holds and the -19 flip would proceed on a false assumption. The register is the tracking surface — a condition is not cleared until its named gate records it resolved.

---

*Line numbers cited are current as of repo `main` on 2026-09-11. All nine ticket statuses re-verified as "To Do" in Jira on the same date. §9 kill-switch framing re-verified against `services/killswitch.js` on 2026-09-11.*
