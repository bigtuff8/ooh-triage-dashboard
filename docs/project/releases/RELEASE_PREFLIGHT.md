...
panel: Independent Release-preflight panel run 2026-09-16. Hard-blocking lenses Critical-Thinker, Compliance, Security and Technical all returned SHIP; advisory Transition returned SHIP. No blockers. Every load-bearing code claim was re-verified against merged main (config.js:90 fail-closed; control.js consume-before-write; api.js:164 dispatch requireAuth-only + :370 admin iot-gate; api.js:49 controlLive derived; requireMinRole genuinely absent from code; WRITES_DISABLED still "true", replicas:1 pinned). The verdict was confirmed an honest, complete NOT-YET-GO that closes no register row and lets no verbal sign-off clear a probe. Advisory findings folded in before presentation: B0/R0 flagged partially-cleared (residual carried by B2); replicas:1 pin + Recreate/maxSurge:0 deploy strategy made explicit C5 flip-checklist lines; ON->OFF abort-visibility signal promoted to build-required; and the kill-switch's Cosmos dependency vs the Cosmos-independent env-lock fallback stated in S6.
...
Note on B0/R0: it is PARTIALLY cleared — the recognisable site NAME shipped (PR #15, tests 1-3), but its residual slice (brand + full postal address at confirm) is carried by **B2** above. Do not read B0 as fully closed; its residual greens when B2 greens.
...
The flip PR checklist MUST additionally carry, as explicit merge-blocking lines (not prose elsewhere):
- **replicas: 1 remains pinned** (C2 single-use holds only at replicas:1; scale-out requires a shared single-use store first).
- **Deployment strategy is Recreate (or RollingUpdate with maxSurge: 0)** so no two pods answer /api/me with divergent controlLive during the flip or the abort — at replicas:1 a surge>0 rollout briefly runs old (OFF) and new (ON) pods together on divergent config.
...
- **ON->OFF "returned to safety-lock" signal — BUILD-REQUIRED before the flip.** The R8/S10 signal covers OFF->ON; the reverse (re-disable / abort) is scaffolded and additive. An abort operators cannot see undermines the reversibility this whole gate rests on, so this is build-required (not decide-or-accept): build it so a re-disable/abort is as visible as go-live, or record an explicit justification for why an invisible abort is acceptable.
...
- **S6 — pod-roll duration = abort SLA, and the two abort levers' trade-off.** Measure the rolling-restart duration and state it as the abort SLA. There are two abort levers with opposite trade-offs: (a) the **iot-gated kill-switch** (routes/api.js:370; services/killswitch.js setGlobal) is INSTANT and runtime-effective, BUT it is a Cosmos-doc write and writesBlocked() reads that doc — so it depends on Cosmos health; (b) the **env re-disable** (WRITES_DISABLED back to true) is Cosmos-INDEPENDENT (config.writesDisabled is checked first and synchronously, config.js:90) BUT is a pod-roll, NOT instant. The shakedown abort trigger must name the kill-switch as the instant lever, treat Cosmos health as a monitored precondition for it, and name the env re-disable as the Cosmos-independent fallback (with its pod-roll latency = the S6 SLA).
