...
**Status:** FIRST DRAFT + 2026-09-25 Sam/Spencer validation round + 2026-09-25 controlled bench test folded in. Working position, not signed-off fact. Every capability claim is either (a) grounded in a cited live read / SD ticket / repo doc / the validation round / the bench test, or (b) explicitly flagged as inferred/unverified. The topology and comms picture is now largely confirmed (Q1/Q2/Q3/Q6 resolved); **control-execution is proven only for the Tuya mechanism on the bench and is dormant in prod, and control CONFIRMATION for Salus is still unproven** (Q4/Q8 now carry controlled bench-test results, 2026-09-25; Q7 remains tbc, low priority). Do not build against the still-flagged rows until §7 clears them.

**Changelog:**
- 2026-09-25 — updated with **controlled bench-test results (Q4/Q8)**. Tuya control **mechanism proven end-to-end on the bench** (`switchDesired` → `pending` → **`synced`** in ~2.4–2.6s on `bench-owon-1` and `bench-tongou-sy1-1`, clean capture-and-restore; the `switchDesired` contract key is the same for both vendors — the bridge abstracts the per-vendor DP) **but DORMANT in prod** (prod runs `integration-bridge:prod-v3`, built 2026-07-23, which predates the Tuya command fix commit `5b1ca84` of 2026-08-28 by ~5 weeks; the fix ships only in the `uat-sd664-*` bench images — so Sam's belief that Tuya command is live is incorrect: prod needs a bridge redeploy to a fixed image before command works). The `switchSyncStatus`/`setpointSyncStatus` **value domain is confirmed** — `pending` → `synced` (success) / `failed` (vendor rejected) / `rejected` (refused / out-of-range), with `synced` positively observed live on Tuya. **Salus control confirmation remains UNPROVEN**: a captured Salus setpoint write (+1°C then restored) on the online, healthy bench device `spencer-home-salusit700` was **not consumed** — `setpointSyncStatus` stayed frozen at a stale `pending` from 2026-09-17 — so the Salus command path is itself dormant on the bench. All cited inline as "(controlled bench test, 2026-09-25)".
...
**Status and confidence.** This is a first draft for James, now carrying the 2026-09-25 Sam/Spencer validation round. It is honest about confidence: the **topology and comms picture is now largely confirmed** (Sam/Spencer validation round, 2026-09-25 — the gateway dependencies, the Salus receiver identity, the AMR external-API reclassification, and the two-boiler split are settled); the read plane is largely proven (live sweep 2026-09-25 + prod reads 2026-09-16); but the **control-execution and confirmation plane is only partially proven**. The controlled bench test (2026-09-25) has now shown that the **Tuya control mechanism works end-to-end on the bench** (`switchDesired` → `pending` → `synced` in ~2.4–2.6s) but is a **confirmed no-op in production today** because the prod bridge image predates the Tuya command fix (research-sd-core §5.4; §7 Q4); that the **`*SyncStatus` value domain is confirmed** (`pending` → `synced` / `failed` / `rejected`); but that **Salus control confirmation is still unproven** — a captured Salus setpoint write on a healthy bench device was not consumed (§7 Q8). Treat every "control" claim as provisional until §7 closes; in particular, treat Salus control as unconfirmed/indeterminate until a real Salus `synced` is observed.
...
|---|---|---|---|---|---|---|---|---|---|---|
| C1 | `tuya Profile` (**3,089; 2,719 active** — SD-446 companion §2, corrects the 3103 sweep figure) | Tuya smart switch / relay (kitchen equip, lighting, fans, over-door heaters) | `fryer`,`grill`,`combioven`,`merrychef`,`bainmarie`,`heatedgantry`,`externallighting`,`extractfan`,`barfans`,`overdoorheater`, etc. | Tuya (Tongou / OWON) cloud | Path 2 (TB-native Pulsar). Sense C only (logical). Needs Sense A (R10A) for site internet but NOT as data hop | `active`; telemetry `bizCode`=online/offline; `device_status`; `signalStrength`; fresh `switch_1`/power (research-tb-sweep §3) | `switchDesired` → `switchReported` + `switchSyncStatus`. NB Tuya `switchReported`/`switchSyncStatus` are **written but currently unread estate-wide** — OOH would be the first reader (research-sd-core §5.7) | `switchDesired: true/false`. Normalise on-state: `switchReported` → `switchOn` → `switch_1` (cross-vendor variance, OOH_TB_DIRECT_DESIGN §4b) | 🟡 **mechanism PROVEN on bench, DORMANT in prod.** The Tuya control mechanism now works end-to-end on the bench: `bench-owon-1` (vendor DP `switch_1`) and `bench-tongou-sy1-1` (vendor DP `switch`) both went `switchDesired` → `pending` → **`synced`** in ~2.4–2.6s with clean capture-and-restore, and the OOH/TB contract key is the SAME (`switchDesired`) for both vendors — the bridge abstracts the per-vendor DP (controlled bench test, 2026-09-25). **BUT command is a NO-OP in prod today:** prod runs `integration-bridge:prod-v3` (built 2026-07-23), which predates the Tuya command fix (commit `5b1ca84`, 2026-08-28) by ~5 weeks; the fix ships only in the `uat-sd664-*` bench images (research-sd-core §5.4, §6). **Sam's belief that Tuya command is live is incorrect — prod needs a bridge redeploy to a fixed image first.** Do NOT present as controllable until that redeploy. NB **read/telemetry is native & correct** via the Tuya PE integration (3,089 devices, SD-446 companion §2, §6) — it is only *command in prod* that is dormant | Kitchen / lighting / ventilation |
| C2 | `salusDevice` (thermostat share of 1181) | Salus **IT700** thermostat (heat-only, e.g. Flaming Grill CH1) | `salusit700` | Salus IT700 via **AWS IoT** (Device Shadow) → bridge | Path 1 (bridge). On-site **receiver** (`-gateway`, §3.3 G4) wires to the boiler and actuates it — usually simple on/off; the thermostat sends its instructions to the receiver. **Schedules are stored LOCALLY in the thermostat** (Sam/Spencer validation round, 2026-09-25). Consequence: on/off actuation depends on the receiver, but schedules do not | `isOnline`=true, `connectivity`=LIVE, fresh `localTemperature`, `lastMessageRSSI` | **`setpointSyncStatus`** (`synced` vs `pending`/`failed`); chain `setpointDesired`→`heatingSetpoint`→`setpointReported` | `setpointDesired`(°C), `modeDesired`(heat only). Boost = single setpoint write | 🟢 read live (SD-446 🟢228) — **but control CONFIRMATION is UNPROVEN.** On the online, healthy bench device `spencer-home-salusit700`, a captured setpoint write (+1°C then restored) was **not consumed** — `setpointSyncStatus` stayed frozen at a stale `pending` from 2026-09-17 while readback telemetry stayed live — so the Salus command path is itself dormant on the bench (controlled bench test, 2026-09-25). Treat Salus control as unconfirmed/indeterminate until a real Salus `synced` is observed; see 6261 `pending` anomaly (§7 Q8) | Heating |
...
**Value domain CONFIRMED (controlled bench test, 2026-09-25):** the `switchSyncStatus` / `setpointSyncStatus` domain is now confirmed as **`pending` → `synced`** (success) | **`failed`** (vendor rejected the write) | **`rejected`** (refused / out-of-range). `synced` was positively observed live on Tuya (~2.5s). This closes the value-domain question (previously inferred from SD-492); what remains open is whether the Salus command path itself advances `pending` → `synced` (§7 Q8).
...
- **C1 Tuya** — the control **mechanism is now proven on the bench** (`switchDesired` → `pending` → `synced` in ~2.4–2.6s, both `switch_1` and `switch` vendor DPs — controlled bench test, 2026-09-25), so the mechanism itself is no longer indeterminate. **But in prod it is a confirmed no-op** — the prod bridge image (`integration-bridge:prod-v3`, 2026-07-23) predates the Tuya command fix (commit `5b1ca84`, 2026-08-28) (§3, research-sd-core §5.4). So Tuya control is **dormant in prod today** and must not present as controllable until the prod bridge is redeployed to a fixed image.
...
- **C2/C3 Salus** — `setpointSyncStatus` is designed as a real ack, **but the Salus command path is itself dormant and its confirmation is UNPROVEN.** On the online, healthy bench device `spencer-home-salusit700`, a captured setpoint write (+1°C then restored) was **not consumed** — `setpointSyncStatus` stayed frozen at a stale `pending` from 2026-09-17 while readback telemetry stayed live (controlled bench test, 2026-09-25). A real Salus `synced` has **never** been observed. Site 6261's `salusit700-1` shows `setpointSyncStatus=pending` (desired 19 / reported 21.5) → read-OK, control-NOT-confirmed (research-tb-sweep §3) — the exact worked case for the indeterminate state. **OOHDASH-102 reframe (controlled bench test, 2026-09-25):** because a Salus setpoint stuck at `pending` is now **reproducible on an ONLINE bench device with a healthy receiver**, 6261's stuck `pending` may be a **Salus command-path / command-formation problem, not (only) the offline receiver** — we may not be sending the correct command and/or not reading the correct confirming telemetry. Treat Salus control as unconfirmed/indeterminate until a real Salus `synced` is observed.
...
|---|---|---|---|---|
| C1 Tuya | `switchDesired` (bool) | `switchReported` → `switchOn` → `switch_1` | `switchSyncStatus` | mechanism proven on bench (`synced` ~2.5s, both vendor DPs); prod no-op until bridge redeploy |
| C2/C3 Salus | `setpointDesired`(°C), `modeDesired`, `hwBoostHoursDesired`(int) | `setpointReported`, `modeReported`, `hwBoostReported` | `setpointSyncStatus`, `modeSyncStatus`, `hwBoostSyncStatus` | designed as real ack, but a real `synced` never observed; bench write not consumed (stuck `pending`) — confirmation UNPROVEN |
...
|---|---|---|---|---|
| `gk-6261-salusit700-1` | C2 (Salus IT700 thermostat) | LIVE — `active=True`, `connectivity`=LIVE, fresh temp | **Control-reachable but currently INDETERMINATE** — `setpointSyncStatus=pending` (desired 19 / reported 21.5). **OOHDASH-102 reframe (controlled bench test, 2026-09-25):** a Salus setpoint stuck at `pending` is now reproducible on an ONLINE bench device with a healthy receiver, so this stuck `pending` may be a **Salus command-path / command-formation problem, not (only) the offline receiver** | Offer setpoint/mode; surface last write as "sent, not yet confirmed" — **not** success |
...
| `gk-6261-fryer-1` (+ other Tuya appliances) | C1 (Tuya switch) | LIVE — `active=True`, online, reporting power | **Dormant — control is a NO-OP in prod today.** The mechanism is proven on the bench (`synced` ~2.5s, controlled bench test 2026-09-25) but prod runs a bridge image that predates the fix | Read on/off + power; **do NOT present a working switch** until the prod bridge is redeployed to a fixed image |
...
Per James's steer: state the **working position**, then list each open item. After the 2026-09-25 Sam/Spencer validation round, **Q1, Q2, Q3 and Q6 are RESOLVED** and their answers have been folded into the fact sections (§2, §3, §5). The 2026-09-25 controlled bench test then resolved **Q4** (Tuya control mechanism proven on the bench but **dormant in prod** — bridge redeploy needed) and confirmed the **Q8** `*SyncStatus` value domain (`pending`/`synced`/`failed`/`rejected`) — **but a real Salus `synced` was not observed, so Salus control confirmation remains unproven**. **Q5 has moved to Jonathan/Rikki (James picking up directly)**; **Q7 remains tbc (low priority)**.

**Operational flags from the 2026-09-25 controlled bench test (for follow-up — out of scope for this model):**
- **Bench/dev-TB credentials live in Key Vault `airedale-kv-uat`** (`thingsboard-dev-username` / `thingsboard-dev-password`), **not** `airedale-kv-prod`.
- **The dev/bench TB is no longer the isolated 8-device playground the runbook describes** — it now holds ~3,333 devices, including production-style `gk-####` names. The isolation premise should be **re-verified with Spencer** before relying on the bench being safely separated from prod.
...
| **Q4 — RESOLVED (with a prod caveat)** (controlled bench test, 2026-09-25) | **Tuya control mechanism + prod-enablement.** Does the Tuya command mechanism work, and when does prod carry it? | **RESOLVED — mechanism works, but prod is DORMANT.** The control mechanism works end-to-end on the bench: `bench-owon-1` (vendor DP `switch_1`) and `bench-tongou-sy1-1` (vendor DP `switch`) both went `switchDesired` → `pending` → **`synced`** in ~2.4–2.6s with clean capture-and-restore; the OOH/TB contract key is the SAME (`switchDesired`) for both vendors — the bridge abstracts the per-vendor DP. **BUT command is DORMANT in prod:** prod runs `integration-bridge:prod-v3` (built 2026-07-23), which predates the Tuya command fix (commit `5b1ca84`, 2026-08-28) by ~5 weeks; the fix ships only in the `uat-sd664-*` bench images. **Sam's belief that Tuya command is live is incorrect — prod needs a bridge redeploy to a fixed image first.** Model Tuya as: read = live; control mechanism = proven; control in prod = **DISABLED until the bridge is redeployed.** Present Tuya as **read + on/off state, control DISABLED** in prod. (Read side already settled: native Tuya PE, 3,089 devices / 2,719 active.) | Prod bridge redeploy owner (Spencer) |
...
| **Q8 — value domain CONFIRMED, Salus confirmation still UNPROVEN** (controlled bench test, 2026-09-25) | **`*SyncStatus` value domain + a real Salus `synced`.** Only `pending` had been observed live; `synced`/`failed`/`rejected` were inferred from SD-492, not seen. | **Value domain CONFIRMED:** `switchSyncStatus`/`setpointSyncStatus` = **`pending` → `synced`** (success) / **`failed`** (vendor rejected) / **`rejected`** (refused / out-of-range); `synced` positively observed live on Tuya (~2.5s). **BUT a real Salus `synced` was NOT observed:** on the online, healthy bench device `spencer-home-salusit700`, a captured setpoint write (+1°C then restored) was **not consumed** — `setpointSyncStatus` stayed frozen at a stale `pending` from 2026-09-17 while state-readback telemetry stayed live. So the Salus command path is itself dormant on the bench. **Salus control confirmation remains unproven — treat Salus control as unconfirmed/indeterminate until a real Salus `synced` is observed.** This also reframes OOHDASH-102 (see §4.3, §5.2): 6261's stuck `pending` may be command-formation, not only receiver-offline. **UX steer (Spencer):** confirmation must be an **asynchronous background task that pops up when the confirmation is read — not a blocking wait** for the operator. | Bench / Spencer |
...
```json
{
  ...
  "control": {
    ...
    "statusToday": "read-live-control-unconfirmed",
    "caveats": ["value domain confirmed pending/synced/failed/rejected (bench 2026-09-25)", "a real Salus synced NEVER observed — bench write on spencer-home-salusit700 not consumed, stuck pending", "6261 currently pending/unconfirmed — may be command-formation not only receiver-offline (OOHDASH-102 reframe)"]
  },
  "confidence": "read=proven; control=value-domain confirmed (bench 2026-09-25) but Salus synced unproven — treat as indeterminate until a real synced is observed (Q8)"
}
```
...
*End of draft, updated 2026-09-25 with the Sam/Spencer validation round and the 2026-09-25 controlled bench test. Confidence summary: the **topology and comms picture is now largely confirmed** (Q1 Salus receiver, Q2 gateway dependencies, Q3 AMR-as-external-API, Q6 two distinct boiler classes all resolved); the **read plane is largely proven**; but **control-execution is proven only for the Tuya mechanism on the bench and is DORMANT in prod, and control CONFIRMATION for Salus is still unproven** — the Tuya command mechanism works end-to-end on the bench but the prod bridge image predates the fix so prod command is a no-op until a redeploy (Q4); the `*SyncStatus` value domain (`pending`/`synced`/`failed`/`rejected`) is confirmed but a real Salus `synced` was never observed, so Salus control stays indeterminate until one is (Q8); Intesis has no observed ack and an unresolved comms-path conflict (Q7, low priority); and the boiler is non-standard with its migration now owned by Jonathan/Rikki (Q5). Sections 4 (delivery semantics) and 5 (resolution) remain the load-bearing behavioural additions. Validate the remaining open items in §7 before building against them.*
