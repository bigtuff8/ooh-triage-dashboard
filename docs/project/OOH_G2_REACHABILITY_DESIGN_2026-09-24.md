...
SECTION: Label-match segment rule
The group label is resolved on the SAME split-segment basis as the anchor rule, so build cannot pick a different matching approach and silently fall to generic. Take normaliseName(d.name), split on hyphen/whitespace (norm.split(/[-\s]+/).filter(Boolean)), and read the equipment-type segment as the segment immediately PRECEDING the r10a/gw gateway token; for the Salus hub the equipment-type token is the salusit700 segment. Map that one token via an explicit lookup (boiler→"boiler", maindb→"main distribution board", kitchen→"kitchen", salusit700→"heating") to natural English. If no mapped token is found, use the safe generic fallback "the equipment group" and NEVER render the raw token. This segment-extraction rule is exact and mandatory.
SECTION: Operator resolution paths
The group-offline and gateway-less states are new outcome states, so each needs an operator resolution path that makes sense — the earlier design left the operator on the anchor-down question, which is nonsensical when the anchor is up or absent. After a group-offline amber callout (anchor UP) the operator is NOT asked "Is the Lighthouse unit back online?"; instead they are offered a drop-to-capture for the affected equipment group, because the site is reachable but that group cannot be reached remotely — so the OOH action is to log a ticket carrying the group context. At a gateway-less site the anchor-specific outcomes are replaced with per-device liveness for the equipment the current flow concerns, then the normal capture path, with NO Lighthouse-unit framing in any chip or line. Both reuse the existing outcomeCaptured path; no remote-control affordance is offered for unreachable equipment.
SECTION: Multi-group offline
When two or more group gateways are offline at once (6770-shaped), the design renders ONE combined amber callout enumerating the affected groups, not N stacked identical alerts. The "other equipment can still be reached" reassurance line is stated once, not repeated per group. The verbatim combined wording is fixed so build cannot improvise it.
SECTION: Accepted assumptions
G2 rests on two recorded bets. First: every remote command reaches the site through the lwgateway (the LoRaWAN hub), so the lwgateway alone governs the site-reachable claim. It is drawn from existing tb-device/flows commentary and the 2026-09-24 read-only sweep of 6 sites (5198, 6770, 6261, 4631, 6886, 6748); it is NOT verified against every possible estate architecture. Re-validate before deploy if new site types or connectivity architectures are commissioned. Second (the 48h freshness timing bet): both the per-equipment auto-divert and the group-offline read key on devices reading offline, which under OOHDASH-85's freshness model only happens once lastActivityTime crosses the ~48h threshold, so there is a window after a group gateway fails during which neither the divert fires nor the group reads offline, and an operator may attempt to control unreachable equipment. This is accepted as INHERITED from the freshness model; fully closing it would need the scoped-out per-flow direct-gateway checks. Re-examine if per-type freshness thresholds are tuned.
...
DECISION: Approve this revised G2 design — the exact siteConnectivityAnchor segment-equality rule, the group-gateway-offline check pinned to the ONE connectivity handler (reached via the existing kitchen/heating/hot-water auto-diverts, with lighting/fan explicitly out of scope for their own tickets), the exact segment-extraction label-match rule (segment immediately before the r10a/gw/gateway token, mapped via lookup, safe generic else, never the raw token), the operator resolution path after a group-offline callout (drop-to-capture reusing outcomeCaptured, NO anchor-down question, NO remote-control affordance), the gateway-less resolution path (per-device liveness then normal capture, NO Lighthouse-unit framing in any chip or line), the ONE combined amber callout for 2+ groups offline with the verbatim multi-group wording, the recorded lwgateway AND ~48h-freshness-window accepted bets, the >1-lwgateway loser disposition, the verbatim gateway-less and group-offline operator copy, AND the added UI/UX design (keep the two-column grid with the shortened sidebar, style the reachability/assessment conclusion as a severity-coloured callout above the deciding question, and meet the stated kitchen-readability intent verified by screenshot) — all consistent with E's side-card removal? | Yes, approve | Request changes
-->
...
### Multi-group offline — ONE combined amber callout (CX-lens item)

When two or more group gateways read offline in the same connectivity result (a 6770-shaped site where, e.g., both `boiler-r10a` and `maindb-r10a` are down at once), the design renders **exactly one** amber `.alert.warn` callout that **enumerates the affected groups**, NOT N stacked identical alerts. The "other equipment can still be reached" reassurance line is stated **once**, not repeated per group. This keeps the operator's eye on one conclusion at 2am rather than a wall of near-identical amber boxes.

- **Verbatim operator-facing line (2+ groups offline, anchor up):** "The site is reachable, but two of its equipment groups aren't responding — the **boiler** and the **main distribution board** — because each has lost its own connection. Other equipment at this site can still be reached."
- **Build rule for the enumeration.** Collect the natural-English labels of every offline group (using the label lookup), then render them as a single grammatical list inside the one callout: two groups joined with " and " (as above); three or more as a comma list with " and " before the last (e.g. "the **boiler**, the **kitchen** and the **main distribution board**"). Open with "two of its equipment groups aren't responding" / "three of its equipment groups aren't responding" (the count spelled out) so the sentence reads naturally. The reassurance clause "Other equipment at this site can still be reached." closes the single callout and appears once.
- **Verbatim caller-facing line (2+ groups offline):** "The site itself is online, but a couple of the units that run your equipment have lost their connection. I can't reach that equipment remotely, so I'll log it as a priority for the IoT team."
- The single-group verbatim copy above still applies when exactly one group is offline; the combined form fires only at 2+.

### Operator resolution path after the group-offline callout (anchor UP)

The operator must NOT be shown the anchor-down question ("Is the Lighthouse unit back online?") — it is nonsensical when the anchor is online. **Decision (this is the design):** after the amber group-offline callout (single- or multi-group), the operator is offered a **drop-to-capture for the affected equipment group(s)**. The site is reachable, but that equipment group cannot be reached remotely, so the OOH action is to log a ticket carrying the group context (which group(s), that its gateway is unreachable). **No remote-control affordance is offered for unreachable equipment** — there is no "turn it on / retry" chip, because the gateway is down and no command can land.

- **Exact chips shown** (below the amber callout and its `.stepq`):
  - `<button class="chip" data-testid="group-offline-capture" onclick="flowStep({ofq:'gcap'})">Log it for the IoT team</button>`
  - `<button class="chip" data-testid="group-offline-back" onclick="flowStep({ofq:'gback'})">The caller says it's working again</button>` — the recovery affordance, so an operator whose caller reports the kit back is not forced to log a false outage. This does NOT offer control; it re-runs the connectivity read.
- **Exact `.stepq` above the chips:** "The [group]/these equipment groups can't be reached remotely while their connection is down. Log it for the IoT team, or re-check if the caller says it's working again."
- **Resulting outcome — reuses the existing capture path**, the same outcomeCaptured helper (flows.js line 234) the connectivity, kitchen and heating flows already call, not a new one. On the "Log it for the IoT team" chip (`flowStep({ofq:'gcap'})`) build calls outcomeCaptured with key `gcap` and these exact fields: subject = "Equipment group gateway offline — [label(s)] unreachable"; detail = "Site reachable, but the [label(s)] equipment group gateway(s) are offline; that equipment cannot be reached remotely. No remote action possible; needs IoT connectivity investigation" followed by the existing "(raised from [from] flow)" suffix when `f.data.from` is set (mirroring line 633); script = "The site itself is online, but the unit that runs that equipment has lost its connection, so I can't reach it remotely tonight. I've logged it as a priority for the IoT team."; OohCaptureClass = "connectivity". The [label(s)] interpolation carries the natural-English group label(s) via the lookup — never the raw token — and enumerates all affected groups in the multi-group case (same grammatical list as the callout).

- `flowStep({ofq:'gback'})` → re-runs the connectivity read once (`f.data.ofq` cleared, connectivity handler re-entered): if the group gateway now reads online the operator falls through to the reachable outcome; if still offline the same amber callout and chips are shown again. This mirrors the existing "It's back online" recovery behaviour without ever asserting a Lighthouse-unit frame.

This resolution reuses the exact `outcomeCaptured` mechanism the kitchen/heating/connectivity flows already use (flows.js lines 234-239, 631-636), so build implements no new outcome type.
...
**Exact segment-extraction rule (mandatory — build must not pick a different matching approach).** The label is resolved on the SAME split-segment basis as the anchor rule, so there is exactly one matching mechanism across G2:

1. Compute `segments = normaliseName(d.name).split(/[-\s]+/).filter(Boolean)` — identical to the anchor helper and to `classifyDevice` at `services/tb-device.js` line 303.
2. Find the index of the **gateway token** — the first segment equal to `r10a` or `gw` (the `-r10a` hardware gateways use `r10a`; a `gw`-tokened gateway uses `gw`). The **equipment-type token is the segment immediately PRECEDING it** (`segments[i-1]`). For `gk-6770-boiler-r10a-1` the segments are `[gk, 6770, boiler, r10a, 1]`, the gateway token `r10a` is at index 3, so the equipment-type token is `segments[2]` = `boiler`.
3. **Salus hub special case:** the Salus hub name `gk-<site>-salusit700-gateway-1` splits to `[gk, <site>, salusit700, gateway, 1]` — the gateway token here is the literal `gateway` segment, and the equipment-type token is the segment preceding it, `salusit700`. So the same "segment immediately before the gateway token" rule holds, with the gateway token being `r10a`, `gw`, or `gateway`.
4. Map that single extracted equipment-type token through the explicit lookup:
   - `boiler` → **"boiler"**
   - `maindb` → **"main distribution board"**
   - `kitchen` → **"kitchen"**
   - `salusit700` (the Salus-hub token) → **"heating"**
5. **If no gateway token is found, or the preceding segment is not in the lookup, use the safe generic fallback "the equipment group" — and NEVER render the raw token** (`boiler-r10a`, `salusit700`, any `gk-…` string) to the operator.

This exact rule (find the `r10a`/`gw`/`gateway` token, take the segment immediately before it, map via the lookup, else safe generic) is mandatory: it mirrors the `siteConnectivityAnchor` segment-equality basis so build cannot silently fall to a substring approach or a different position and land on the wrong label or a raw token.

The old, looser phrasing is superseded: take the offline group gateway's `normaliseName(d.name)`, split on hyphen/whitespace, extract the equipment-type token by the exact rule above, and match it against the lookup. If a future group gateway carries a token not in the lookup, fall back to a safe generic ("the equipment group") rather than ever emitting the raw namespace token — the copy MUST NEVER render `boiler-r10a` or any `gk-…` string to the operator.
...
### Operator resolution path at a gateway-less site (build contract)

The anchor-specific outcomes at `public/js/flows.js` lines 620-636 — the "Is the Lighthouse unit back online?" question ("It's back online / lights on" chip, "Still dead — capture & escalate" chip) and the two anchor-framed captures at lines 622-636 — are **replaced** in the gateway-less branch (anchor `null`) with options that make sense when there is no Lighthouse unit. **No chip and no line references a Lighthouse unit, a wall-mounted unit, a gateway, or a fuse-board/router check** — nothing must reference a gateway the site does not have. The operator goes to per-device liveness for the equipment the current flow concerns, then the normal capture path.

- **Exact operator line above the step (the info callout):** the verbatim gateway-less line "This site uses direct device connections — we'll check the equipment directly." (rendered as `.alert.info`, per the UI/UX section — neutral, no site verdict).
- **Then per-device liveness for the flow's equipment.** The connectivity handler reads `d.online` on the specific equipment the current flow concerns (`f.data.from` carries the originating flow; the relevant devices are the ones that flow already reads). It states, in plain English, whether that specific unit is reachable — e.g. "The thermostat you're calling about is responding." or "The thermostat you're calling about isn't responding right now." — with **no** whole-site claim and **no** gateway framing.
- **Exact chips shown** (below that per-device line):
  - `<button class="chip" data-testid="gwless-capture" onclick="flowStep({ofq:'dcap'})">Log it for the IoT team</button>`
  - `<button class="chip" data-testid="gwless-back" onclick="flowStep({ofq:'dback'})">The caller says it's working again</button>` — re-runs the per-device read; no control affordance.
- **Exact `.stepq` above the chips:** "We check this site's equipment directly. Log it for the IoT team, or re-check if the caller says it's working again." — no Lighthouse-unit language.
- **Resulting outcome — reuses the existing capture path**, the same outcomeCaptured helper (flows.js line 234). On the "Log it for the IoT team" chip (`flowStep({ofq:'dcap'})`) build calls outcomeCaptured with key `dcap` and these exact fields: subject = "Equipment unreachable — direct-connection site"; detail = "This site uses direct device connections (no site hub). The equipment the caller reported is not responding on its own connection. No remote action possible; needs IoT connectivity investigation" followed by the existing "(raised from [from] flow)" suffix when `f.data.from` is set; script = "This site's equipment connects directly rather than through a single hub, and the unit you're calling about isn't responding, so I can't reach it remotely tonight. I've logged it as a priority for the IoT team."; OohCaptureClass = "connectivity". The script and detail carry NO Lighthouse-unit / gateway framing — the copy MUST NOT reference a gateway this site does not have.

- `flowStep({ofq:'dback'})` → re-runs the per-device liveness read once; if the unit now reads online the operator falls to a reachable statement, otherwise the same options are shown again. No control affordance, no Lighthouse framing at any point.

This reuses the exact `outcomeCaptured` mechanism (flows.js lines 234-239) and introduces no new outcome type; it only ensures nothing references a gateway the site does not have.
...
- **The ~48h freshness window bounds how quickly a group-gateway failure surfaces (ACCEPTED BET, inherited from OOHDASH-85).** Both the routes into the group-offline conclusion depend on devices reading offline: the per-equipment auto-divert fires only when the equipment devices themselves read offline (kitchen line 489, heating lines 330/342, hot-water line 451), and the group-offline enumeration fires only when the group gateway itself reads offline. Under the OOHDASH-85 freshness model, `online` is derived by `deriveFreshnessOnline` (`services/tb-device.js` line 506): a device — INCLUDING a group gateway, which carries its own `lastActivityTime` and is mapped through the same `online:` assignment at lines 585-587 as any other device — reads offline only once its own `lastActivityTime` age crosses the ~48h `FRESHNESS_DEFAULT_MS` threshold (the per-type override map at line 586 is present but currently empty, so ~48h applies to gateways too). **Consequence:** there is a window after a group gateway fails during which its downstream equipment devices have not yet gone stale AND the gateway's own telemetry has not yet gone stale — so neither the per-equipment auto-divert fires nor the group reads offline, and an operator may attempt to control equipment that is in fact unreachable. This is **accepted as INHERITED from the freshness model**, not a G2 defect: fully closing it would require the per-flow direct-gateway liveness checks that are explicitly scoped OUT of G2. **Re-examination trigger:** re-examine this bet if per-type freshness thresholds are tuned (i.e. if a shorter override is added for gateway-kind devices in the `freshnessThresholdFor` map), which would narrow the window. Recorded here in the same form as the lwgateway bet so it is not re-litigated at build time.
...
- **Group-offline resolution path (anchor up).** After the amber group-offline callout, assert the operator is NOT shown the anchor-down question ("Is the Lighthouse unit back online?"). Assert the drop-to-capture chips are shown (`group-offline-capture`, `group-offline-back`), that `group-offline-capture` lands on an `outcomeCaptured` ticket carrying the group context (which group, gateway unreachable), and that NO remote-control affordance (turn-on/retry) is offered for the unreachable equipment.
- **Gateway-less resolution path (anchor null).** Assert the anchor-specific outcomes (lines 620-636) are replaced with per-device-liveness-then-capture options (`gwless-capture`, `gwless-back`), that the `outcomeCaptured` ticket fires with no Lighthouse-unit framing, and that NO chip or line references a Lighthouse unit, wall unit, gateway, or fuse-board/router check — nothing references a gateway the site does not have.
- **Multi-group offline → ONE combined amber callout.** On a 6770-shaped site with 2+ group gateways offline at once, assert exactly ONE amber `.alert.warn` callout is rendered (not N stacked identical alerts), that it enumerates all affected groups in natural English as a single grammatical list, and that the "Other equipment at this site can still be reached." reassurance line appears exactly once.
- **Label-match segment rule.** Unit-test the label extraction: for `gk-6770-boiler-r10a-1` the extracted equipment-type token is the segment immediately before `r10a` (`boiler`) mapping to "boiler"; `maindb`→"main distribution board"; `kitchen`→"kitchen"; the Salus hub `gk-<site>-salusit700-gateway-1` → segment before `gateway` (`salusit700`) → "heating". An unmapped token → the safe generic "the equipment group", never the raw token.
- **48h freshness timing window (recorded-bet awareness).** Not a defect to fail on, but the test stage should be aware: a group gateway that has just failed will not read offline (and the per-equipment divert will not fire) until its `lastActivityTime` age crosses ~48h; construct a fixture with a group gateway whose `lastActivityTime` is < and > the threshold and confirm the group-offline callout appears only in the stale case, documenting the window as the accepted inherited limitation.
