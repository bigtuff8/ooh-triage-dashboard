...
**Changelog:** 2026-09-25 — updated with Sam/Spencer validation-round answers (Q1/Q2/Q3/Q6 resolved and moved into section 3 as confirmed fact; Q4/Q8 now under bench verification; Q5 moved to Jonathan/Rikki with James picking it up; Q7 low-priority tbc).
...
The important nuance: the **map of what is where and how it connects is now largely confirmed**, and the **reading** side is proven. What still needs live confirmation is the **control** side — actually sending an instruction and getting back honest proof it landed (see section 4). The technical document is careful about that distinction, and so is this one.

---

## 4. What is still an open question

The technical document lists eight questions, labelled Q1 to Q8. The 2026-09-25 Sam/Spencer validation round **answered four of them** — Q1, Q2, Q3 and Q6 are now RESOLVED, and their answers have moved up into section 3 as confirmed fact (they are kept in the table below, marked RESOLVED, so you can still see what was asked and where to look it up). The rest are still open: Q4 and Q8 are now being confirmed on a test bench, Q5 has moved to Jonathan and Rikki (with James picking it up), and Q7 remains to be confirmed. Nothing still open is a defect to fix blindly — several are decisions or definitions to agree, not bugs.
...
| **Q4 — under bench verification** (in progress, 2026-09-25) | The smart switches (Tuya brand) — when will the live system actually be able to command them, and are we even sending the right command? | The capability is built but not switched on in the live system yet, and we are now checking on the test bench whether we send the correct command and read the correct confirmation back. Reading works fine; only sending is still being proven. Until then the dashboard shows Tuya as read-only. | Bench / Spencer | Any Tuya device, e.g. `gk-6261-externallighting-1` or `gk-6261-fryer-1` (bench references: `bench-owon-1`, `bench-tongou-sy1-1`). |
...
| **Q8 — under bench verification** (in progress, 2026-09-25) | For the heating that *does* give a confirmation signal, we have only ever seen it say "pending" (in progress). We have not yet seen it say "confirmed" or "failed" for real. | We are watching a real instruction complete on the bench before relying on "confirmed" as proof. Two things came out of the review: (1) the confirmation should arrive as a **background pop-up when it is read — the operator should never be made to wait** on a spinner; and (2) the stuck "pending" might mean we are sending the wrong command or reading the wrong confirmation, not just that a box is offline. | Bench test / Spencer | `gk-6261-salusit700-1` currently shows `setpointSyncStatus: pending` — observe a real write actually reach `synced` (and what `failed` / `rejected` look like), ideally on bench `spencer-home-salusit700` or a controlled live write. |
...
Put simply: **the map of what is where and how it connects is now largely confirmed, and we can trust what the dashboard READS. The ability to CONTROL equipment, and to CONFIRM that control actually worked, is still being verified.**
...
The 2026-09-25 validation round with Sam and Spencer already turned four of the questions (Q1, Q2, Q3, Q6) into confirmed fact. The remaining items in section 4 are the work from here:
...
- **Spencer** (technical / bridge owner) — the remaining travel-and-confirmation questions land with you: Tuya command (Q4) and the heating confirmation signal (Q8) are on the test bench, and the air-conditioning confirmation plus the connection-path conflict (Q7) are yours to settle (low priority).
